...
public class ClaimDataUnmarshaller implements Processor {
    ...
    public void process(Exchange exchange) {
        ...
        claimContentWrapper = new ClaimContentWrapper();
        claimContentWrapper.setClaimImport(exchange.getMessage().getBody(ClaimImport.class));
        exchange.getMessage().setBody(claimContentWrapper);
        ...
        claimContentWrapper.setEhImportClaimData(unmarshalledEhClaimData.getMessage().getBody(ImportClaimData.class));
        logServiceClaim.logUnmarshall(exchange);

    }

    private Exchange unmarshallClaimData(Exchange exchange) {
        Exchange marshalContent = ExchangeBuilder.anExchange(exchange.getContext()).withBody(claimContentWrapper.getClaimImport().getContent())
                .withProperty(Constants.CLAIM, claimContentWrapper.getClaim()).build();
