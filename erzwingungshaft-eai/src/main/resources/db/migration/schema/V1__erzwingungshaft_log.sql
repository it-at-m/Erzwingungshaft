...
CREATE TABLE eh_log.erzwingungshaft_entry
(

    id                   SERIAL PRIMARY KEY,       -- extern: public UUID
    eh_uuid              UUID,
...
CREATE INDEX idx_entry_id ON eh_log.erzwingungshaft_entry(id);
CREATE INDEX idx_entry_geschaeftspartner_kassenzeichen ON eh_log.erzwingungshaft_entry (geschaeftspartner_id, kassenzeichen);

CREATE TABLE eh_log.erzwingungshaft_data
(
    ...
    CONSTRAINT fk_entry FOREIGN KEY (entry_id) REFERENCES eh_log.erzwingungshaft_entry (id) ON DELETE CASCADE

);

CREATE INDEX idx_unmarshalled_entry_id ON eh_log.erzwingungshaft_data(entry_id);

CREATE TABLE eh_log.erzwingungshaft_log
(
...
CREATE INDEX idx_log_entry_id ON eh_log.erzwingungshaft_log(entry_id);

CREATE TABLE eh_log.erzwingungshaft_xml
(
    id         SERIAL PRIMARY KEY,  -- INT PK
    entry_id   INTEGER NOT NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX idx_xml_entry_id ON eh_log.erzwingungshaft_xml(entry_id);

CREATE TABLE eh_log.erzwingungshaft_content
(
...
CREATE INDEX idx_content_entry_id ON eh_log.erzwingungshaft_content(entry_id);
