...
public class XtaServiceWrapper {
     ...
     *
     * <p>
     * Zweck der Methode ist der Abholvorgang aus der Message-Box: <em>„Mit der Methode getMessage holt der Leser eine Nachricht vom Empfänger ab.“</em>
     * (XTA 2, Version 3.1.1, Abschnitt 5.4.3.2). In der aktuellen Spezifikationsversion erfolgt die Selektion ausschließlich über die {@code MessageID}.
     * </p>
     *
     * <p><b>SOAP-Struktur</b> (vgl. 5.4.3.2 „Operation getMessage“):</p>
     * <ul>
     *   <li><b>Header (Input)</b>: {@code AuthorIdentifier} – Typ {@code oscimeta:PartyType}</li>
     *   <li><b>Body (Input)</b>: {@code MsgBoxFetchRequest} – inkl. {@code MsgSelector} mit {@code wsa:MessageID}</li>
     *   <li><b>Header (Output)</b>: {@code oscimeta:MessageMetaData} und {@code osci:MsgBoxResponse}</li>
     *   <li><b>Body (Output)</b>: {@code xta:GenericContentContainer} (Fachnachricht inkl. Attachments)</li>
     * </ul>
     *
     * <p>
     * Hinweis: Der Status der abgeholten Nachricht wird beim Empfänger erst nach Quittierung via {@code close} auf „abgeholt“ gesetzt
     * (vgl. 5.4.3.2.1: „…der Status auf 'abgeholt' geändert wird, nachdem die Transaktion durch Aufruf der Methode &lt;close&gt; bestätigt worden ist.“).
     * </p>
     *
     ...
     * @see <a href="#close(java.lang.String,de.muenchen.erzwingungshaft.xta.dto.XtaIdentifier)">close</a> zur Quittierung
     */
    public XtaMessage getMessage(String messageId, XtaIdentifier consignorIdentifier) throws XTAWSTechnicalProblemException, PermissionDeniedException, InvalidMessageIDException {
    ...
    /**
     * Quittiert die Abholung von Nachrichten bzw. (Teil-)Listen und gibt serverseitige Ressourcen frei ({@code close}).
     *
     * <p>
     * Ziel der Methode ist es sicherzustellen, „dass Nachrichten oder Listen nicht mehrfach verarbeitet werden“
     * (XTA 2, Version 3.1.1, Abschnitt 5.4.3.3). Die Empfangsquittierung soll zeitnah erfolgen; mit {@code close}
     * wird die zugehörige Transaktion beendet und z. B. ein Iterator freigegeben.
     * </p>
     *
     * <p><b>SOAP-Struktur</b> (vgl. 5.4.3.3 „Operation close“):</p>
     * <ul>
     *   <li><b>Header (Input)</b>: {@code AuthorIdentifier} – Typ {@code oscimeta:PartyType}</li>
     *   <li><b>Body (Input)</b>: {@code MsgBoxCloseRequest} – i. d. R. mit der zugehörigen {@code wsa:MessageID}/Ressourcenkennung</li>
     ...
     */
    public void close(String messageId, XtaIdentifier consignorIdentifier) throws XTAWSTechnicalProblemException, PermissionDeniedException, InvalidMessageIDException {
    ...
    /**
     * Ruft Statusinformationen zu eingegangenen Nachrichten ab ({@code getStatusList}) und liefert eine (Teil-)Liste
     * von {@code MessageID}s inkl. Metadaten.
     *
     * <p>
     * Der Leser kann die Ergebnisliste über Selektionskriterien (z. B. Zeitraum, „nur neue“) einschränken und anhand
     * der zurückgelieferten Metadaten (Autor, Leser, Betreff, {@code MessageID} etc.) entscheiden, welche Nachrichten
     * im nächsten Schritt per {@code getMessage} abgeholt werden (XTA 2, Version 3.1.1, Abschnitt 5.4.3.1).
     * </p>
     *
     * <p><b>SOAP-Struktur</b> (vgl. 5.4.3.1 „Operation getStatusList“):</p>
     * <ul>
     *   <li><b>Header (Input)</b>: {@code AuthorIdentifier} – Typ {@code oscimeta:PartyType}</li>
     ...
     *   <li><b>Body (Output)</b>: {@code osci:MsgStatusListType} (enthält u. a. {@code oscimeta:MessageMetaData})</li>
     * </ul>
     *
     * <p><b>Implementierungshinweis</b>: Diese Methode kombiniert die OSCI-Antwortbestandteile zu einem Domänenobjekt
     * {@link XtaStatusListing} (Mapping von {@code MsgStatusListType} sowie {@code MsgBoxResponseType}).</p>
     *
     ...
     *
     * <p>
     * Die Methode dient dazu festzustellen, „ob ein Leser prinzipiell für einen Dienst elektronisch erreichbar ist“
     * (z. B. via Verzeichnisdienste wie DVDV). Das Ergebnis unterscheidet u. a. {@code ServiceIsAvailable},
     ...
     * @throws XTAWSTechnicalProblemException bei technischen Problemen auf Seiten der XTA-Transportinfrastruktur
     * @throws PermissionDeniedException wenn kein autorisierter Zugriff vorliegt
     * @throws ParameterIsNotValidException wenn {@code ServiceType} oder {@code ReaderIdentifier} ungültig sind
     */
    public LookupServiceResponse lookupService(String service, XtaIdentifier reader, XtaIdentifier clientIdentifier) throws XTAWSTechnicalProblemException, PermissionDeniedException, ParameterIsNotValidException {
        return xtaServicePorts
                .managementPortType()
                .lookupService(
     ...
     * Konfigurationsänderungen oder bei technischen Problemen (XTA 2, Version 3.1.1, Abschnitt 5.4.1.1).
     * </p>
     *
